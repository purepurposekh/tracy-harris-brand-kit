/*
 * The Freedom Filled® Pathway Audit · scoring engine v1
 * Machine truth: projects/ffb-strategy/pathway-audit/SCORING-MATRIX.md (tracy-harris-co repo)
 * Pure logic, no DOM. Loaded by pathway-audit.html, testable in node.
 *
 * Model: every option carries one focus area (of 9), a readiness value 0-3,
 * optional objection tags, optional F.R.E.S.H flag. Q24 is the readiness gate:
 * counts in overall readiness R only, feeds no area.
 * Nothing numeric is ever shown to the respondent.
 */

(function (root, factory) {
  if (typeof module === 'object' && module.exports) { module.exports = factory(); }
  else { root.AuditEngine = factory(); }
  ...
  function selectRoute(R, NA, q24, DF, q1IsA, q7IsA) {
    if (DF >= 4) return 'fresh';
  ...
  /* ---------- full computation ----------
   * answers: array of 24 option indices (0-3), in question order.
   */
  function compute(answers) {
    ...
    var route = selectRoute(R, NA, q24, DF, answers[0] === 0, answers[6] === 0);
    var band = selectBand(R);

    // F.R.E.S.H signal level
    var freshLevel = DF >= 4 ? 'first' : (DF >= 2 ? 'elevated' : 'quiet');
